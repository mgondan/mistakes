...
rolog::consult(system.file("pl/tratio.pl", package="mistakes"))

Q <- quote(search(tratio(d, mu, s_d, n), .S, .P))
...
for(i in 1:length(S))
{
  ...
  solutions[i] = all(Pi == "expert")
  explanations[i] = paste(Pi, collapse=", ")
}
...
Hamilton Rating Scale for Depression (HDRS, range from best = 0 to worst = 42).
The significance level is set to $\alpha = 5\%$ two-tailed.

````{r, echo=FALSE}
Tab <- rbind(
  c(T0=t0, EOT=eot, `$D$`=d),
  c(T0=s_t0, EOT=s_eot, `$D$`=s_d))
  
knitr::kable(Tab, digits=1)
````

Does rfCBT lead to a relevant reduction (i.e., more than `r mu` units) in mean
...
\begin{aligned}
  ...
  = \frac{`r fmt(d, 2)` - `r mu`}{`r fmt(s_d, 2)`/\sqrt{`r n`}}
  = `r questions[solutions][1]`.
\end{aligned}
$$

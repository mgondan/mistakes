...
Q <- quote(search(pvalue, pvalue(d, mu_d, s_d, n, alpha), .S, .R, .P, .E))
...
for(i in 1:length(S))
{
  ...
  p2 <- paste(praise("pvalue", Pi), collapse=" ")
  b2 <- paste(blame("pvalue", Pi), collapse=" ")
  e3 <- paste(feedback("pvalue", Ei), collapse=" ")
  if(e3 != "")
...
Solution
========
The $p$-value is calculated by: `r mathml(S[[which(solutions)[1]]]$S)`
